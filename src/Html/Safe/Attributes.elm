...
-}
form : String -> Attribute { compat | form : Supported } msg
form =
    coerce Core.form
...
-}
max : String -> Attribute { compat | max : Supported } msg
max =
    coerce Core.max
...
-}
min : String -> Attribute { compat | min : Supported } msg
min =
    coerce Core.min
...
-}
step : String -> Attribute { compat | step : Supported } msg
step =
    coerce Core.step
...
-}
cols : Int -> Attribute { compat | cols : Supported } msg
cols =
    coerce Core.cols
...
-}
rows : Int -> Attribute { compat | rows : Supported } msg
rows =
    coerce Core.rows
...
-}
wrap : String -> Attribute { compat | wrap : Supported } msg
wrap =
    coerce Core.wrap
...
a query string.
-}
ismap : Bool -> Attribute { compat | ismap : Supported } msg
ismap =
    coerce Core.ismap
...
E.g. `"#planet-map"`.
-}
usemap : String -> Attribute { compat | usemap : Supported } msg
usemap =
    coerce Core.usemap
...
-}
shape : String -> Attribute { compat | shape : Supported } msg
shape =
    coerce Core.shape
...
-}
challenge : String -> Attribute { compat | challenge : Supported } msg
challenge =
    coerce Core.challenge


{-| Specifies the type of key generated by a `keygen`. Possible values are:
rsa, dsa, and ec.
-}
keytype : String -> Attribute { compat | keytype : Supported } msg
keytype =
    coerce Core.keytype
...
`tr`.
-}
align : String -> Attribute { compat | align : Supported } msg
align =
    coerce Core.align
...
-}
cite : String -> Attribute { compat | cite : Supported } msg
cite =
    coerce Core.cite
...
-}
target : String -> Attribute { compat | target : Supported } msg
target =
    coerce Core.target
...
directly.
-}
download : Bool -> Attribute { compat | download : Supported } msg
download =
    coerce Core.download


{-| Indicates that clicking an `a` and `area` will download the resource
directly, and that the downloaded resource with have the given filename.
-}
downloadAs : String -> Attribute { compat | downloadAs : Supported } msg
downloadAs =
    coerce Core.downloadAs


{-| Two-letter language code of the linked resource of an `a`, `area`, or `link`.
-}
hreflang : String -> Attribute { compat | hreflang : Supported } msg
hreflang =
    coerce Core.hreflang


{-| Specifies a hint of the target media of a `a`, `area`, `link`, `source`,
or `style`.
-}
media : String -> Attribute { compat | media : Supported } msg
media =
    coerce Core.media
...
-}
ping : String -> Attribute { compat | ping : Supported } msg
ping =
    coerce Core.ping
...
For `a`, `area`, `link`.
-}
rel : String -> Attribute { compat | rel : Supported } msg
rel =
    coerce Core.rel
...
For `del`, `ins`, `time`.
-}
datetime : String -> Attribute { compat | datetime : Supported } msg
datetime =
    coerce Core.datetime


{-| Indicates whether this date and time is the date of the nearest `article`
ancestor element. For `time`.
-}
pubdate : String -> Attribute { compat | pubdate : Supported } msg
pubdate =
    coerce Core.pubdate
...
-}
reversed : Bool -> Attribute { compat | reversed : Supported } msg
reversed =
    coerce Core.reversed


{-| Defines the first number of an ordered list if you want it to be something
besides 1.
-}
start : Int -> Attribute { compat | start : Supported } msg
start =
    coerce Core.start
...
For `td` and `th`.
-}
colspan : Int -> Attribute { compat | colspan : Supported } msg
colspan =
    coerce Core.colspan
...
-}
headers : String -> Attribute { compat | headers : Supported } msg
headers =
    coerce Core.headers
...
For `td` and `th`.
-}
rowspan : Int -> Attribute { compat | rowspan : Supported } msg
rowspan =
    coerce Core.rowspan
...
colgroup, rowgroup.
-}
scope : String -> Attribute { compat | scope : Supported } msg
scope =
    coerce Core.scope
